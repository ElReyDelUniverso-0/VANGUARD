...
async function gdelt(query: string, max: number): Promise<RadarItem[]> {
  const url =
    ...
    encodeURIComponent(`${query}&mode=artlist&maxrecords=${max}&format=json&sort=datedesc`);
  try {
    const j = (await fetchJson(url, 12_000)) as {
    ...
    return []; // fuente caída = sección vacía, la guerra sigue
  }
}
...
async function collect(): Promise<CacheShape> {
  const [gdeltEs, gdeltEn, gdacsItems] = await Promise.all([
    gdelt(
  ...
  return { t: Date.now(), gdeltEs, gdeltEn, gdacs: gdacsItems };
}
